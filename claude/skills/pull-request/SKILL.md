...
A PR body has one reader: someone without your context, who will skim anything long. **Tell the story** — what was wrong, what you did about it, and what they should push back on — in about 250 words. Length does not buy care from a reviewer; it spends it.
...
2. **Disclose the risk.** What did *not* change, and what the suite says — "No production behavior changes here; the suite is unchanged at 327 passing." One sentence, and the one a reviewer leans on to decide how hard to look.
...
| Before | After |
| --- | --- |
| <img src="<url-1>" width="420" alt="Settings panel, before"> | <img src="<url-2>" width="420" alt="Settings panel, after"> |
```
...
- **Title** matches `^(fix|feat|docs|style|refactor|perf|test|build|ci|chore|revert)(\(.+\))?!?: [^A-Z].*$`, which is what `lint-pr.yml` enforces.
...
- **The body carries no AI attribution**, per the global instructions, which cover PR titles and summaries alongside commit messages.
