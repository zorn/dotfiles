...
Build each sentence to carry one idea, in the spirit of ASD-STE100 Simplified Technical English — the aerospace controlled-language standard for writing a non-native reader must not misread. Adopt its writing rules, not its restricted dictionary: prefer short sentences, the active voice, and the present tense; hold subordinate clauses down; and use one term for one concept, rather than naming the same thing an "issue" on one line and a "ticket" on the next. This governs the sentence, where the paragraphs above govern what to include and in what order.
